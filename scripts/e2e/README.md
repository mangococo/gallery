...
- **31 框选多选（marquee）**：真实输入拖拽成框（e2e.ts `drag` 步骤）→ 命中/计数/卡片选中态与框选矩形一致 →
  拖拽中 overlay 几何（跨脚本步骤不断链、截图留证）→ 释放消失 → Esc 取消回滚 → 亚阈值放行点击（开灯箱）→
  ⌘ 增量框选 → 反方向替换 → 真实点击/右键菜单回归 → 批量工具条 → 首页 columns 布局框选。
  断言口径：拖拽端点取整（MouseEvent 坐标为整数）+ 卡片矩形原始浮点 + 端点离卡片边缘 ≥6px（亚像素漂移免疫）
- **32 删除旅行迁移**：删除带照片旅行 → 专用弹窗（默认「一并移入回收站」/「迁移到其他旅行」）→ 选目标 →
  迁移并删除 → 时间线源旅行消失、目标旅行计数合并 → 目标旅行页照片数/墙卡片数吻合 →
  Esc 取消不删 → 默认直删路径（带照片）回归
- **33 伪装扩展名 HEIC**：HEIC 内容 + .jpg 扩展名（微信转存形态，gen-demo sips 造样张）→
  缩略图经 ftyp 内容嗅探走 WASM 回退上墙 → 灯箱大图协议层转 JPEG → 信息面板文件名断言
