...
- **34 旅行日期自动重算**：多选 2 张 05-01 照片迁入新建旅行 → 新旅行日期自动填充、
  源旅行 startDate 顺延 05.02（endDate 手动值 05.04 锁定不动）→ 迁回一张 startDate 回摆 05.01；
  覆盖「对齐即自动」三态：空字段填充 / 自动字段跟随 / 手动字段锁定（决策逻辑单测在 trip-dates.test.ts）
- **35 软件更新**：设置 → 关于区当前版本 → 检查更新（GALLERY_E2E 下主进程注入 mock fetcher
  返回 v99.0.0，链路不真实访问网络）→ 发现新版 + 发布说明摘要 → 前往下载
  （E2E 下 openExternal 只记日志）→ toast 断言
